...
//> Functions interpreter-constructor
  ...
  Interpreter() {
    globals.define("clock", new ConyoCallable() {
      @Override
    ...
    globals.define("make_sabi", new ConyoCallable() {
      @Override
      ...
      @Override
      public String toString() { return "<native fn make_sabi>"; }
    });

    globals.define("make_salita", new ConyoCallable() {
      @Override
      public int arity() { return 1; }
      ...
      @Override
      public String toString() { return "<native fn make_salita>"; }
    });

    globals.define("make_basa", new ConyoCallable() {
      @Override
      public int arity() { return 1; }
      ...
      public Object call(Interpreter interpreter,
        ...
        Object s = null;
        if (( (String)arguments.get(0)).equals("%s") ) {
          Scanner sc = new Scanner(System.in);
          s = sc.nextLine();
        }

        if (( (String)arguments.get(0)).equals("%d") ) {
          Scanner sc = new Scanner(System.in);
          s = sc.nextDouble();
        }
...
/* Evaluating Expressions interpret < Statements and State interpret
  void interpret(Expr expression) {
    try {
      ...
      Conyoscript.runtimeError(error);
    }
  }
*/
...
//> Inheritance interpret-superclass
    ...
    if (stmt.superclass != null) {
      ...
      if (!(superclass instanceof ConyoClass)) {
...
//> interpret-methods
    ...
    Map<String, ConyoFunction> methods = new HashMap<>();
    for (Stmt.Function method : stmt.methods) {
/* Classes interpret-methods < Classes interpreter-method-initializer
      ConyoFunction function = new ConyoFunction(method, environment);
*/
...
/* Classes interpret-methods < Inheritance interpreter-construct-class
    ConyoClass klass = new ConyoClass(stmt.name.lexeme, methods);
*/
//> Inheritance interpreter-construct-class
    ConyoClass klass = new ConyoClass(stmt.name.lexeme,
        (ConyoClass)superclass, methods);
...
/* Functions visit-function < Functions visit-closure
    ConyoFunction function = new ConyoFunction(stmt);
*/
/* Functions visit-closure < Classes construct-function
    ConyoFunction function = new ConyoFunction(stmt, environment);
*/
...
//> check-is-callable
    if (!(callee instanceof ConyoCallable)) {
      throw new RuntimeError(expr.paren,
          "Can only call functions and classes.");
    }

//< check-is-callable
    ConyoCallable function = (ConyoCallable)callee;
//> check-arity
...
//> Inheritance interpreter-visit-super
  ...
  public Object visitSuperExpr(Expr.Super expr) {
    ...
    ConyoClass superclass = (ConyoClass)environment.getAt(
        distance, "super");
//> super-find-this
...
//> super-find-method
    ...
    ConyoFunction method = superclass.findMethod(expr.method.lexeme);
//> super-no-method
